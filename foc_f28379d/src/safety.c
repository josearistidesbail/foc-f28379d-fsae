...
// Bench bypass for the SW undervoltage trip (restored 2026-07-23). When 0 the UV
// check is skipped -- used while the DC-bus sensor scale is untrusted at low bench
// voltage. Seeded from UV_FAULT_EN_DEFAULT (bench => 0, production => 1); live-
...
void safety_check_isr(void)
{
    ...
    // refs.vbus is the CONTROL value: the fixed override when g_vbus_override_v > 0
